...
function Dashboard() {
    ...
    const carregarDados = async () => {
        ...
        try {
            const pDashboard = api.get(`/dashboard/${usuarioLogado.id}`).catch(() => ({ data: { saldoAtual: 0 } }));
            const pInvest = api.get(`/analytics/investimentos/${usuarioLogado.id}`).catch(() => ({ data: [] }));

            const hoje = new Date();
            const mesesOffsets = [-2, -1, 0, 1, 2];
            
            const pExtratos = mesesOffsets.map(offset => {
                const dataAlvo = new Date(hoje.getFullYear(), hoje.getMonth() + offset, 1);
                const mes = String(dataAlvo.getMonth() + 1).padStart(2, '0');
                const ano = dataAlvo.getFullYear();
                
                return api.get(`/extrato/usuario/${usuarioLogado.id}?mes=${mes}&ano=${ano}`)
                    .then(res => ({ label: `${mes}/${ano}`, offset, dados: res.data }))
                    .catch(() => ({ label: `${mes}/${ano}`, offset, dados: [] }));
            });

            const [resDashboard, resInvest, ...extratos] = await Promise.all([pDashboard, pInvest, ...pExtratos]);

            const extratoAtual = extratos.find(e => e.offset === 0).dados;
            const receitas = extratoAtual
                .filter(t => t.tipoFluxo === 'RECEITA')
                .reduce((acc, curr) => acc + curr.valor, 0);
            
            const despesas = extratoAtual
                .filter(t => t.tipoFluxo === 'DESPESA' || t.formaPagamento === 'CREDITO')
                .reduce((acc, curr) => acc + curr.valor, 0);

            setResumo({
                saldoAtual: resDashboard.data?.saldoAtual || 0,
                totalReceitas: receitas,
                totalDespesas: despesas
            });

            const graficoGastos = extratos.map(ext => {
                const aVista = ext.dados
                    .filter(t => (t.tipoFluxo === 'DESPESA' || t.tipo === 'DESPESA') && t.formaPagamento !== 'CREDITO')
                    .reduce((acc, curr) => acc + curr.valor, 0);
                    
                const credito = ext.dados
            ...
            setHistoricoGastos(graficoGastos);
            setPerformanceInvest(resInvest.data || []);
            setCarregando(false);
    ...
    const hojeLabel = new Date().toLocaleDateString('pt-BR', {month: '2-digit', year: 'numeric'});
        ...
        <div className="animate-fadeIn pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                ...
                <div className="bg-gray-900 p-6 rounded-2xl border border-emerald-500/20 shadow-2xl">
                    <p className="text-[10px] text-gray-500 uppercase font-tech mb-1">Entradas em {nomeMesAtual}</p>
                ...
                <div className="bg-gray-900 p-6 rounded-2xl border border-red-500/20 shadow-2xl">
                    <p className="text-[10px] text-gray-500 uppercase font-tech mb-1">Despesas em {nomeMesAtual}</p>
                    ...
                    <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={historicoGastos} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                ...
                                <ReferenceLine x={hojeLabel} stroke="#6366f1" strokeDasharray="3 3" />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
